...
output_path = "track_diff_linelevel"
linelevel_path = 'linelevel_raw/qt_base.csv'
...
with open(linelevel_path, 'r') as tmp:
    with open(os.path.join(output_path, "qt_base_ex.csv"), 'a', newline='') as tmp2:
        ...
        for row in reader:
            ...
            # else:
            #     encoded = quote(row[CSVCol.FILEDIR.value], safe='')
            #     file_name = f"{int(patch_num) + 1}_{encoded}.json"
            #     for big_r in big_ranges:
            #         if big_r[0] <= change_num <= big_r[1]:
            #             big_dir = f"{big_r[0]}-{big_r[1]}"
            #             break
            #     for small_r in small_ranges:
            #         if small_r[0] <= change_num <= small_r[1]:
            #             small_dir = f"{small_r[0]}-{small_r[1]}"
            #             break
            #     target_dir = os.path.join("../raw_data/review/qt", big_dir, small_dir, str(change_num), file_name)
            #     try:
            #         f = open(target_dir, 'r')
            #         json_data = json.load(f)
            #     except FileNotFoundError:
            #         with open(os.path.join(output_path,"log.txt"), "w") as log:
